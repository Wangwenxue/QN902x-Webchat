...
// wenxue	
#if BLE_WX_SENSOR
    app_wxps_create_db(app_wxps_env->features);
#endif
...
// wenxue		
    ...
    if (enabled == true)
    {
        app_wxps_env->conhdl = conhdl;
        app_wxps_enable_req(conhdl, PERM_RIGHT_ENABLE, PRF_CON_NORMAL, PRF_CLI_STOP_NTFIND, 
                            /* app_wxps_env.features & WXPS_BODY_SENSOR_LOC_CHAR_SUP */ HRS_LOC_CHEST);
        app_wxps_env->enabled = true;
    }
    else if (app_wxps_env->enabled == true)
    {
...
uint8_t app_set_scan_rsp_data(uint16_t srv_flag)
{
    ...
    if (srv_flag == 0)
    {
      ...
      || BLE_WX_SENSOR|| BLE_BP_SENSOR || BLE_HR_SENSOR || BLE_GL_SENSOR || BLE_TIP_SERVER || BLE_SP_SERVER\
...
// wenxue
#ifdef UART0_QPPS_ENABLE   //ericyang20160218 disable QPPS FEE9 UUID SCAN/ INSTEAD OF WECHAT FEE7	
#else			
#if BLE_QPP_SERVER
    ...
    if (len <= remain_len)
    {
        if (srv_flag & BLE_WX_SENSOR_BIT)
        {
            app_env.scanrsp_data[len+0] = (uint8_t)(ATT_SVC_WX_IOT&0x00FF);
            app_env.scanrsp_data[len+1] = (uint8_t)(ATT_SVC_WX_IOT>>8);
            len += 2;
        }
    }
    else
...
#if BLE_WX_SENSOR
		// manufaccturer :0xFF
    app_env.scanrsp_data[len+0] = 0x09;
    app_env.scanrsp_data[len+1] = 0xFF;
    app_env.scanrsp_data[len+2] = 0x06;
    app_env.scanrsp_data[len+3] = 0x00;
		
		nvds_tag_len_t name_length = 6;
		uint8_t tt[6]={0};
		
		if (NVDS_OK == nvds_get(NVDS_TAG_BD_ADDRESS, &name_length, &tt[0]))
		{		
        	for(uint8_t i=0;i<6;i++)
             	app_env.scanrsp_data[len+4+i] = tt[5-i];		
		}
    len += 9+1;
		#else
    // Slave connection interval range:0x12
