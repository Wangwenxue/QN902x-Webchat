...
{
#if 0
	static uint16_t bleHeadLen = sizeof(BlueDemoHead);  //nicole
	CString send_msg_1 ={SEND_HELLO_WECHAT,sizeof(SEND_HELLO_WECHAT)}; //nicole
	uint32_t length_1 = 0;  //nicole
	BaseRequest basReq_1 = {NULL}; //nicole
	static uint8_t fix_head_len_1 = sizeof(BpFixHead);//nicole
	BpFixHead fix_head_1 = {0xFE, 1, 0, htons(ECI_req_auth), 0};//nicole
	uint8_t *data=NULL; //nicole
#endif	
    ...
    default:
        ...
        app_eaci_msg_hdl((src_id >> 8) & 0xFF,
                                ...
                                ind->data);
#elif QN_DEMO_MENU
////////////////////////////此处为测试代码，不加app_task_msg_hdl不会自己跑到app_task_msg_hdl处理程序内

		app_task_msg_hdl(msgid, ind);
        if (ind->len != 0)
        {
            memcpy(app_env.input, ind->data, ind->len);
						app_env.len=ind->len;
//						uart_write(QN_DEBUG_UART, ind->data,ind->len, NULL);
//            app_menu_hdl();
        }
	...
	#ifdef UART0_QPPS_ENABLE
		//uart_write(QN_DEBUG_UART, ind->data,ind->len, NULL);
		//if(msg_source == SRC_MESSAGE_QPPS)  //ericyang 20160218 check msg source
		//printf("msg_source = %d\r\n", msg_source);

		  if(msg_source == SRC_MESSAGE_WECHAT)
		  {
		  #if 1
		  	if((uart_msg_len + ind->len) >= sizeof(uart_msg))
	  		{
			  	memset(uart_msg, 0x0, sizeof(uart_msg));
				uart_msg_len = 0;
				printf("Recv out of range!\r\n");
			}
	  		else
	  		{
			  	memcpy(uart_msg+uart_msg_len, ind->data, ind->len);
			  	uart_msg_len += ind->len;
				ke_timer_set(WXPS_SDATA_REQ_TIMER,TASK_APP,100);  //ericyang test, delay time need to fixed depend on data length!!!!
		  	}
		#else
			uart_write(QN_DEBUG_UART, ind->data,ind->len, NULL);
		#endif
		  ...
		  else
		  {
			...
			if ((app_qpps_env->char_status >> cnt) & QPPS_VALUE_NTF_CFG)
			{
					...
					app_qpps_data_send(app_qpps_env->conhdl, cnt, app_env.len, app_env.input);
					//QPRINTF("**QPPs send**QPPs,app_qpps_data_send,app_env.len=%d,index =%d\r\n",app_env.len,cnt);  //nicole
			}
			if(++cnt==app_qpps_env->tx_char_num) cnt=0;
                }
	#endif
	
		//printf("**Updated****app_env,app_uart_data_ind_handler(),app_env.len=%d\r\n",app_env.len);  //NICOLE
	    	//	printf("TX Data:\r\n");
				//for(i=0;i<ind->len;i++)
				//{
				    //nicole
					//printf("app_env.input[%d]= %c\r\n",i,app_env.input[i]); //nicole 
		//			printf("%s\r\n",app_env.input);
...
#if BLE_WX_SENSOR
    {WXPS_DISABLE_IND,                      (ke_msg_func_t) app_wxps_disable_ind_handler},
    {WXPS_ERROR_IND,                        (ke_msg_func_t) app_wxps_error_ind_handler},
    {WXPS_MEAS_SEND_CFM,                    (ke_msg_func_t) app_wxps_means_send_cfm_handler},
    {WXPS_CFG_INDNTF_IND,                   (ke_msg_func_t) app_wxps_cfg_indntf_ind_handler},
    {WXPS_ENERGY_EXP_RESET_IND,             (ke_msg_func_t) app_wxps_energy_exp_reset_ind_handler},
    {WXPS_CREATE_DB_CFM,                    (ke_msg_func_t) app_wxps_create_db_cfm_handler},    
		{APP_WXPS_SEND_TIMER,                   (ke_msg_func_t) app_wxps_send_timer_handler},
		{WXPS_AUTH_REQ_TIMER,                   (ke_msg_func_t) app_wxps_dev2wx_proc_timer_handler},
		{WXPS_AUTH_RESP_TIMER,                  (ke_msg_func_t) app_wxps_wx2dev_proc_timer_handler},
		{WXPS_INIT_REQ_TIMER,                   (ke_msg_func_t) app_wxps_dev2wx_proc_timer_handler},
		{WXPS_INIT_RESP_TIMER,                  (ke_msg_func_t) app_wxps_wx2dev_proc_timer_handler},
		{WXPS_SDATA_REQ_TIMER,                  (ke_msg_func_t) app_wxps_dev2wx_proc_timer_handler},
		{WXPS_SDATA_RESP_TIMER,                 (ke_msg_func_t) app_wxps_wx2dev_proc_timer_handler},//nicole
		{WXPS_RDATA_PUSH_TIMER,                 (ke_msg_func_t) app_wxps_wx2dev_proc_timer_handler},
		{WXPS_SVIEW_PUSH_TIMER,                 (ke_msg_func_t) app_wxps_wx2dev_proc_timer_handler},
		{WXPS_SBG_PUSH_TIMER,                   (ke_msg_func_t) app_wxps_wx2dev_proc_timer_handler},
