...
/// ATT DB,Testing and Qualification related flags
    ...
    #if defined(CFG_PRF_WXPS)
        #define BLE_WX_SENSOR       1
        #define TASK_WXPS           CFG_TASK_WXPS
        #define WXPS_DB_SIZE        170 //152
    #else
        #define BLE_WX_SENSOR       0
        #define WXPS_DB_SIZE        0
    #endif // defined(CFG_PRF_WXPS)
    ...
    /// Attribute Server
    #if BLE_RSC_SENSOR || BLE_PAS_SERVER || BLE_CSC_SENSOR || BLE_AN_SERVER || BLE_PROX_REPORTER || BLE_FINDME_TARGET || BLE_HT_THERMOM || BLE_BP_SENSOR || BLE_TIP_SERVER || BLE_HR_SENSOR || BLE_WX_SENSOR || BLE_DIS_SERVER || BLE_ACCEL || BLE_SP_SERVER || BLE_BATT_SERVER || defined(CFG_ATTS)
        #define BLE_ATTS            1
    #else
        #define BLE_ATTS            0
...
/// Size of the heap
    ...
    /// some heap must be reserved for attribute database
       ...
       // wenxue
			 #define BLE_DB_SIZE         (GAP_DB_SIZE+GATT_DB_SIZE+QPPS_DB_SIZE+OTAS_DB_SIZE+HTPT_DB_SIZE+PXPR_DB_SIZE+FMPT_DB_SIZE+DISS_DB_SIZE+ BLPS_DB_SIZE+TIPS_DB_SIZE+WXPS_DB_SIZE+HOGPD_DB_SIZE+SCPPS_DB_SIZE+BASS_DB_SIZE+GLPS_DB_SIZE+RSCPS_DB_SIZE+PASPS_DB_SIZE+CSCPS_DB_SIZE+ANPS_DB_SIZE)
		#else
